...
const selectItem = (event, img) => {
  ...
  if (item === -1) {
    ...
    sliders.splice(img, 1);//splice method img index to Exactly 1 img remove from slider Array
  }
}
var timer
const createSlider = () => {
  ...
  const getDuration = document.getElementById('duration').value || 1000;
  const duration = Math.abs(getDuration);
